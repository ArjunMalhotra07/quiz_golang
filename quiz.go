...
func parseLines(lines [][]string) []structs.Problem {
	ret := make([]structs.Problem, len(lines))
	for i, line := range lines {
		ret[i] = structs.Problem{
			Question: line[0],
...
// ! Asks Questions
func questionaires(problems []structs.Problem, timer *time.Timer) {
	correct := 0
problemLoop:
